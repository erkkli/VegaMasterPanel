...
--   FROM PaidBill
--   WHERE (PaidBill.Comment LIKE '%zayi%'
--          OR PaidBill.Comment LIKE '%ZAYI%')
--     AND PaidBill.PaymentTime >= @Trh1
--     AND PaidBill.PaymentTime <= @Trh2
--     AND ISNULL(Price, 0) = 0
--   UNION SELECT @Sube AS Sube,
--                ISNULL(Sum(CashPayment), 0) AS Cash,
--                ISNULL(Sum(CreditPayment), 0) AS Credit,
--                ISNULL(Sum(TicketPayment), 0) AS Ticket,
--                ISNULL(Sum(OnlinePayment), 0) AS OnlinePayment,
--                0 AS Debit,
--                0 AS ikram,
--                0 AS AcikMasalar,
--                0 AS TableNo,
...
--                0 AS zayi
--   FROM Payment
--   WHERE Paymenttime >= @Trh1
--     AND Paymenttime <= @Trh2
--     AND ISNULL(FastSale, 0)=0
--     AND Payment.TableNo<>'DEBIT'
--   UNION SELECT @Sube AS Sube,
--                0 AS Cash,
--                0 AS Credit,
--                0 AS Ticket,
...
--                0 AS Debit,
--                0 AS ikram,
--                sum(quantity*price) AS AcikMasalar,
--                0 AS TableNo,
--                0 AS Discount,
--                0 AS iptal,
--                0 AS zayi
...
--   WHERE Paymenttime >= @Trh1
--     AND Paymenttime <= @Trh2
--     AND ISNULL(FastSale, 0)=1 --UNION
-- --SELECT
----@Sube as Sube,
----0 as Cash, 0 as Credit, 0 as Ticket,0 AS OnlinePayment, 0 as Debit, ISNULL(Sum(Quantity*OriginalPrice),0) AS ikram,0 as AcikMasalar, 0 as TableNo, 0 as Discount, 0 AS iptal, 0 AS zayi
----FROM PaidBill
----WHERE
----Paymenttime >= @Trh1 AND Paymenttime <= @Trh2 and Price=0 and ProductName Not Like '$%'
----and PaidBill.Comment not LIKE '%ZAYi%' and PaidBill.Comment not LIKE '%ZAYI%'
...
-- --SELECT
----@Sube as Sube,
----0 as Cash, 0 as Credit, 0 as Ticket,0 AS OnlinePayment, 0 as Debit, 0 AS ikram,0 as AcikMasalar, 0 as TableNo, 0 as Discount, ISNULL(Sum(Quantity*OriginalPrice),0) AS iptal, 0 AS zayi
----FROM DeletedBill
----WHERE
----DeletingTime >= @Trh1 AND DeletingTime <= @Trh2 and ProductName Not Like '$%'
-- )
...
--       SUM(Credit) AS Credit ,
--       Sum(Ticket) AS Ticket ,
--       Sum(OnlinePayment) AS OnlinePayment --,Sum(Debit) AS Debit
----,Sum(ikram) AS ikram
----,Sum(AcikMasalar) AS AcikMasalar
----,Sum(TableNo) AS TableNo
----,Sum(Discount) AS Discount
