...
GridMvc = (function ($) {
    ...
    */
    gridMvc.prototype.openFilterPopup = function (self, html) {
        ...
        var baseHeight = $(this).closest('.grid-wrap').height();
        var scrollHeight = $(this).closest('.grid-wrap').prop('scrollHeight');
        var difHeight = baseHeight - scrollHeight;
        ...
        if (this.hasAttribute("data-rendered")) {
            ...
            if (!or && typeof (widget.onShow) != 'undefined') {
                widget.onShow();
                scrollHeight = $(this).closest('.grid-wrap').prop('scrollHeight');
                $(this).closest('.grid-wrap').height(scrollHeight + difHeight);
            }
            return or;
        }
    ...
    */
    ...
    gridMvc.prototype.getFilterWidgetForType = function (typeName) {
        for (var i = 0; i < this.filterWidgets.length; i++) {
            var result = this.filterWidgets[i].getAssociatedTypes();
            var check = 0;
            $.each(result, function (key, value) {
                if (typeName.toString().indexOf(value) >= 0) {
                    check = 1;
                    return false;
                }
            });
    ...
    */
    gridMvc.prototype.applyFilterValues = function (initialUrl, columnName, values, skip) {
        ...
        //Bazı durumlarda eklenecek url boş geliyor. ozamanda initialUrl'in sounda & işareti kalıyor 
        //burda App_Start içinde boş bir parametre olarak görünüyor
        if (initialUrl.length > 0 && url != "")
            initialUrl += "&";
        ...
        if (typeof box != "undefined") {
            do {
                ...
                if (html == target) {
                    ...
                    $(box).closest('.grid-wrap').css("height", "");
                }
...
if (typeof (GridMvc.lang) == 'undefined')
    GridMvc.lang = {};
GridMvc.lang.en = {
    filterTypeLabel: "Sorgulama Biçimi: ",
    ...
    code: 'tr',
    boolTrueLabel: "Evet/Var/Aktif",
    boolFalseLabel: "Hayır/Yok/Pasif",
    ...
    textFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                        ...
                        <select class="grid-filter-type form-control">\
                            <option value="1" ' + (this.value.filterType == "1" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Contains + '</option>\
                            <option value="2" ' + (this.value.filterType == "2" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Equals + '</option>\
...
NumberFilterWidget = (function ($) {
    ...
    numberFilterWidget.prototype.getAssociatedTypes = function () {
        return ["System.Int16", "System.Int32", "System.Double", "System.Decimal", "System.Byte", "System.Single", "System.Float", "System.Int64"];
    };
...
//    enumFilterWidget.prototype.onRender = function (container, lang, typeName, values, cb) {
//        this.cb = cb;
//        this.container = container;
//        this.lang = lang;
//        this.value = values.length > 0 ? values[0] : { filterType: 1, filterValue: "" };//support only one filter value
//        this.renderWidget(typeName);
//        this.registerEvents();

//    };

//    enumFilterWidget.prototype.renderWidget = function (typeName) {
//        alert(typeName);
//        var html = '<label>' + this.lang.filterValueLabel + '</label>\
//                    <ul class="menu-list">\
//                        <li><a class="grid-filter-choose ' + (this.value.filterValue == "GelenEvrak" ? "choose-selected" : "") + '" data-value="GelenEvrak" href="javascript:void(0);">' + 'Gelen Evrak' + '</a></li>\
//                        <li><a class="grid-filter-choose ' + (this.value.filterValue == "GidenEvrak" ? "choose-selected" : "") + '" data-value="GidenEvrak" href="javascript:void(0);">' + 'Giden Evrak' + '</a></li>\
//                    </ul>';
//        this.container.append(html);
//    };
...
EnumFilterWidget = (function ($) {
    ...
    enumFilterWidget.prototype.getAssociatedTypes = function () { return ["App.Contracts.Enums.General"]; };
    ...
    enumFilterWidget.prototype.renderWidget = function (typeName) {
        var html;
        var filterValue = this.value.filterValue;
        var filterValueLabel = this.lang.filterValueLabel;
        $.ajax({
            url: "/AutoComplete/GetEnumValueListByType",
            data: { "typeName": typeName },
            dataType: "json",
            async: false,
            global: false,
            success: function (data) {
                html = '<label>' + filterValueLabel + '</label><ul class="menu-list">';
                $.each(data, function (index, value) {
                    html += '<li><a class="grid-filter-choose ' + (filterValue == value.Value ? "choose-selected" : "") + '" data-value="' + value.Value + '" href="javascript:void(0);">' + value.Text + '</a></li>';
                });
                html += '</ul>';
