...
function IlceWidget() {
    ...
    */
	this.loadCustomers = function () {
		var $this = this;
		$.post("/Lisanslandirma/GetIlce", function (data) {
...
function BankaFilterWidget() {
    /***
    * This method must return type of registered widget type in 'SetFilterWidgetType' method
    */
    this.getAssociatedTypes = function () {
        return ["BankaAdiFilterWidget"];
    };
    /***
    * This method invokes when filter widget was shown on the page
    */
    ...
    */
    ...
    this.renderWidget = function () {
        var html = '<p>Banka Adı Seçiniz:</p>\
                    <select style="width:250px;" class="grid-filter-type bankaadilistesi form-control">\
                    </select>';
        this.container.append(html);
    };
    /***
    ...
    */
    this.fillCustomers = function (items) {
        var customerList = this.container.find(".bankaadilistesi");
        for (var i = 0; i < items.length; i++) {
            customerList.append('<option ' + (items[i] == this.value.filterValue ? 'selected="selected"' : '') + ' value="' + items[i] + '">' + items[i] + '</option>');
        }
    };
