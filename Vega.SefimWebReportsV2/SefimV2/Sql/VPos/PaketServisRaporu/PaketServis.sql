...
declare @Kasa nvarchar(100) = '{KASAKODU}';
		...
		COUNT(*) SAY, 
		SUM(TUTAR) BTUTAR
		--,ISNULL(ADI,'')+'/'+ISNULL(SOYADI,'') PLASIYER 
	...
	LEFT JOIN TBLSPOSPAKETCILER AS P WITH(NOLOCK) ON P.IND=P_PAKETCIIND
		 ...
		 AND BAS.OLUSTURMATARIHI>=@par1 AND BAS.OLUSTURMATARIHI<=@par2
		    ...
		    SATIS_TAHSILAT AS (
                ...
                GROUP BY IADE,ISLEMTIPI,IND,TUTAR,KUR,SATISBASLIKIND,CAST(ACIKLAMA AS NVARCHAR(50)),SAY,BTUTAR
                 )  AS T GROUP BY IADE,P_KAYNAK,SAY,BTUTAR
                )

		SELECT 
		SAY OrderCount,BTUTAR+INDIRIM Total,
