...
function UploadFile() {
    ...
    $.ajax({
        url: '/Banner/Upload',  //Server script to process data
        type: 'POST',
...
function DeleteFile() {
    ...
    var dataString = new FormData(form);
    $.ajax({
        url: '/Banner/Delete',  //Server script to process data
        type: 'POST',
        xhr: function () {  // Custom XMLHttpRequest
            var myXhr = $.ajaxSettings.xhr();
            if (myXhr.upload) { // Check if upload property exists
...
function successHandler_Delete(data) {
    if (data.statusCode == 200) {
        var _src = $("#imgBanner").attr("src");
        $("#imgBanner").attr("data-src", _src);
        alert(data.status);
        ClearUpload();
    }
    else {
...
function successHandler_Upload(data) {
    if (data.statusCode == 200) {    
        $("#imgBanner").attr("data-src", data.NewRow);
        alert(data.status);
    }
    else {
        alert(data.status);
