...
{
    public class UrunAnalizSubePersoneliSatisCRUD
    {
        public static List<UrunAnalizRaporuViewModel> List(DateTime Date1, DateTime Date2, string subeid, string ID, string productGroup)
        {
            ...
            try
            {
                ...
                Parallel.ForEach(dtList, r =>
                {
                    ...
                    {
                        Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlUrunAnalizRaporu/UrunAnalizPersonelSatis.sql"), System.Text.UTF8Encoding.Default);
                    }
                    else if (AppDbType == "3")
                    {
                                    ...
                                    if (!productGroup.Equals(null))
                                    {
                                        ...
                                        else
                                        {
                                            foreach (DataRow SubeR in UrunAnalizRaporuDt.Rows)
                                            {
                                                ...
                                                items.PersonelMiktar = f.RTD(SubeR, "Miktar");
                                                items.personelTutar = f.RTD(SubeR, "Tutar");
                                                Liste.Add(items);
                                            }
                                        }
                                    }
