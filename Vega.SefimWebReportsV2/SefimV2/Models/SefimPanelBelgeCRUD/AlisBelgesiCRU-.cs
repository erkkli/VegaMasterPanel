...
namespace SefimV2.Models.SefimPanelBelgeCRUD
{
    public class AlisBelgesiCRUD
    {
        ...
        public ActionResultMessages Insert(BelgeAlisGiderCreate obj)
        {
     ...
     "VALUES" +
            "(@par1, @par2, @par3, @par4, @par5, @par6, @par7, @par8, @par9, @par10, @par11, @par12, @par13, @par14, @par15, @par16, @par17, @par18, @par19, @par20, @par21, @par22, @par23, @par24, @par25, @par26, @par27, @par28, @par29, @par30, @par31, @par32, @par33, @par34, @par35, @par36, @par37, @par38, @par39, @par40, @par41,@par42,@par43, 0,@par44,@par45)" +
     ...
     "(@par1, @par2, @par3, @par4, @par5, @par6, @par7, @par8, @par9, @par10, @par11, @par12, @par13, @par14, @par15, @par16, @par17, @par18, @par19, @par20, @par21, @par22, @par23, @par24, @par25, @par26, @par27,@par28,@par29,@par30)";
        ...
        public ActionResultMessages Onay(int Id, int UserId, UserViewModel usModel)
        {
            ...
            try
            {
                ...
                if (blg.BelgeTip == BelgeTipi.SayimCikisBelgesi || blg.BelgeTip == BelgeTipi.SayimGirisBelgesi)
                {
                    ...
                    sayzayiblg.BelgeTip = BelgeTipi.SayimCikisBelgesi;
                    ...
                    sayfireblg.BelgeTip = BelgeTipi.SayimCikisBelgesi;
                    ...
                    foreach (var hareket in blg.BelgeHarekets)
                    {
                        ...
                        decimal DepoMiktar = GetStokForDepo(usModel.FirmaID, usModel.DonemID, Convert.ToInt32(hareket.Stok), Convert.ToInt32(usModel.DepoID), hareket.SayimTarihDahil);
        ...
        private string BelgeKaydet(string connString, UserViewModel usModel, BelgeAlisGiderCreate blg)
        {
            ...
            using (VegaBelgeWrapper vegaBelgeWrapper = new VegaBelgeWrapper(connString, "F0" + usModel.FirmaID.ToString(), "D" + usModel.DonemID.ToString().PadLeft(4, '0'), 100))//UserId standart 100 basılsın istendi. Burak Yıkmaz
            {
                ...
                baslik.OzelKod1 = itemssub.Where(a => a.Value == blg.Sube).ToList()[0].Text;//blg.Sube;

                itemskasa = AlisBelgesiCRUD.GetKasaList(usModel.FirmaID);//, ((int)blg.BelgeTip), usModel.SefimPanelZimmetCagrisi);//generic alınmalı
                baslik.OzelKod2 = itemskasa.Where(a => a.Value == blg.Kasa).ToList()[0].Text; //blg.Cari;
                ...
                else if (blg.BelgeTip == BelgeTipi.SayimCikisBelgesi)
                {
                    baslik.BelgeAlSat = VegaBelgeHareketTipi.Giris;//0
                    baslik.Iade = 0;
                }
        ...
        public static List<SelectListItem> GetStokSelectList(int TanimId)
        {
            ...
            try
            {
                ...
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    string VegaDbSqlName = f.RTS(r, "SqlName");
                    string VegaDbSqlPassword = f.RTS(r, "SqlPassword");
                    Query = "SELECT S.STOKKODU + ' - ' + S.MALINCINSI + ' - ' + ISNULL(B.BARCODE,'') AS STOK, S.IND STOKIND from F0" + TanimId + "TBLSTOKLAR S,F0" + TanimId + "TBLBIRIMLEREX B WHERE S.ANABIRIM=B.IND AND STATUS=1";
                    string connString = "Provider=SQLOLEDB;Server=" + VegaDbIp + ";User ID=" + VegaDbSqlName + ";Password=" + VegaDbSqlPassword + ";Database=" + VegaDbName + "";
                    f.SqlConnOpen(true, connString);
                    DataTable AcikHesapDt = f.DataTable(Query, true);
                    if (AcikHesapDt.Rows.Count > 0)
        ...
        public static List<SefimPanelStok> GetStokDetayList(int TanimId, int StokId, int ZimFiyat)
        {
            ...
            try
            {
                ...
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    if (ZimFiyat > 0 && ZimFiyat < 10)
                    {
                        Query = "SELECT S.IND STOKIND,B.BIRIMADI,B.IND BIRIMIND,S.STOKKODU,B.BARCODE,S.MALINCINSI,S.ALISFIYATI,case when " + ZimFiyat + ">0 then B.SATISFIYATI" + ZimFiyat + " else 0 end SATISFIYATI,B.KDVDAHIL,B.KDV from F0" + TanimId + "TBLSTOKLAR S,F0" + TanimId + "TBLBIRIMLEREX B WHERE S.ANABIRIM=B.IND AND STATUS=1 AND S.IND=" + StokId + "";
                    }
                    else if (ZimFiyat == 10)
                    {
                        Query = "SELECT S.IND STOKIND,B.BIRIMADI,B.IND BIRIMIND,S.STOKKODU,B.BARCODE,S.MALINCINSI,S.ALISFIYATI,S.ALISFIYATI as SATISFIYATI,B.KDVDAHIL,B.KDV from F0" + TanimId + "TBLSTOKLAR S,F0" + TanimId + "TBLBIRIMLEREX B WHERE S.ANABIRIM=B.IND AND STATUS=1 AND S.IND=" + StokId + "";
                    }
                    else
                    {
                        Query = "SELECT S.IND STOKIND,B.BIRIMADI,B.IND BIRIMIND,S.STOKKODU,B.BARCODE,S.MALINCINSI,S.ALISFIYATI, B.SATISFIYATI1 SATISFIYATI,B.KDVDAHIL,B.KDV from F0" + TanimId + "TBLSTOKLAR S,F0" + TanimId + "TBLBIRIMLEREX B WHERE S.ANABIRIM=B.IND AND STATUS=1 AND S.IND=" + StokId + "";
                    }
                    string connString = "Provider=SQLOLEDB;Server=" + VegaDbIp + ";User ID=" + VegaDbSqlName + ";Password=" + VegaDbSqlPassword + ";Database=" + VegaDbName + "";
                    f.SqlConnOpen(true, connString);
                    DataTable AcikHesapDt = f.DataTable(Query, true);
        ...
        public BelgeList OnayBekleyenBelgeList(string BelgeTipYetkisi)
        {
            BelgeList blglst = new BelgeList();
            blglst.Belgeler = new List<BelgeAlisGiderCreate>();
            ModelFunctions f = new ModelFunctions();
            ...
            {
                f.SqlConnOpen();
                DataTable dt = f.DataTable("SELECT [Id],[Cari],[BelgeNo],[BelgeKod],[Tarih],[BelgeTutar],[AraToplam],[BelgeKDV],[NetToplam],[BelgeTip],[IskontoTop],[AIsk1],[AIsk2],[AIsk3],[AIsk4],[Vade],[Termin],[Parabirimi],[Kur],[VgFatInd],[Terminal],[Aktarim],[Kapali],[BelgeNot],[PesinAd],[PesinVergiNo],[OzelKod],[Depo],[CariExtId],[KayitTarihi],[CikanDepo],[UID],[OzelKod9],[Personel],[Depozitolu],[OtvToplam],[OdemeTutar],[SonBakiye],[BelgeOzelKod1],[BelgeOzelKod2],[BelgeOzelKod3],[BelgeOzelKod4],[BelgeNoArctos]  FROM  Belge where OnayDurumu=0 and BelgeTip in(" + BelgeTipYetkisi + ")");
                foreach (DataRow r in dt.Rows)
                {
        ...
        public BelgeAlisGiderCreate GetBelge(string ID)
        {
            ...
            {
                f.SqlConnOpen();
                DataTable dt = f.DataTable("SELECT [Id],[Cari],[BelgeNo],[BelgeKod],[Tarih],[BelgeTutar],[AraToplam],[BelgeKDV],[NetToplam],[BelgeTip],[IskontoTop],[AIsk1],[AIsk2],[AIsk3],[AIsk4],[Vade],[Termin],[Parabirimi],[Kur],[VgFatInd],[Terminal],[Aktarim],[Kapali],[BelgeNot],[PesinAd],[PesinVergiNo],[OzelKod],[Depo],[CariExtId],[KayitTarihi],[CikanDepo],[UID],[OzelKod9],[Personel],[Depozitolu],[OtvToplam],[OdemeTutar],[SonBakiye],[BelgeOzelKod1],[BelgeOzelKod2],[BelgeOzelKod3],[BelgeOzelKod4],[OnayDurumu],[Sube],[Kasa],[SablonAdi],[Sablon],[BelgeNoArctos]  FROM  Belge where Id=" + ID);
                foreach (DataRow r in dt.Rows)
                {
                    blg.Id = f.RTI(r, "ID");
                    blg.Cari = f.RTS(r, "Cari").ToString();
        ...
        public string GetArctosBelgeAndCreate(string BelgeNo, UserViewModel usModel)
        {
            ...
            using (VegaBelgeWrapper vegaBelgeWrapper = new VegaBelgeWrapper(connString, "F0" + usModel.FirmaID.ToString(), "D" + usModel.DonemID.ToString().PadLeft(4, '0'), 100))//UserId standart 100 basılsın istendi. Burak Yıkmaz
            {
                ...
                // blgalis.BelgeHarekets = blg.BelgeHarekets;
                BlgNo = ((BelgeAlisGiderCreate)Insert(blgalis).result_OBJECT).Id.ToString();
            }
            return BlgNo;
        }
    }
