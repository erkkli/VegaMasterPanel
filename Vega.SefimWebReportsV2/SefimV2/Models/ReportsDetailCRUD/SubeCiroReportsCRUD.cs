...
{
    public class SubeCiroReportsCRUD
    {
        public static List<KasaCiro> List(DateTime Date1, DateTime Date2, string ID, string filterSubeId)
        {
            ...
            try
            {
                ...
                ff.SqlConnOpen();
                DataTable dt = ff.DataTable("select * from SubeSettings Where Status=1 " + filterSubeId);
                ff.SqlConnClose();
                ...
                try
                {
                    ...
                    Parallel.ForEach(dtList, r =>
                    {
                        ...
                        {
                            Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlReports/SubeCiroReports/SubeToplamCiroReports.sql"), System.Text.Encoding.UTF8);
                        }
                        else if (AppDbType == "2")
                        {
                            Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlReports/SubeCiroReports/SubeToplamCiroReports.sql"), System.Text.Encoding.UTF8);
                                    ...
                                    if (KasaCiroDt.Rows.Count > 0)
                                    {
                                        ...
                                        else
                                        {
                                            foreach (DataRow sube in KasaCiroDt.Rows)
                                            {
                                                ...
                                                items.ToplamCiro = sube["ToplamCiro"].ToString(); //(KasaCiroDt.Rows[0]["ToplamCiro"]).ToString();
