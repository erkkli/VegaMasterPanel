...
{
    public class SubeGarsonPersonelReportsCRUD
    {
            ...
            try
            {
                ...
                Parallel.ForEach(dtList, r =>
                {
                    ...
                    if (AppDbType == "1" || AppDbType == "2")// 1 = yeni şefim, 2 =eski Şefim, 3 = Faster
                    {
                        if (subeid.Equals("0"))
                        {
                            Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlReports/SubeGarsonPersonel/WaiterSales.sql"), System.Text.UTF8Encoding.Default);
                        }
                        else
                        {
                            Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlReports/SubeGarsonPersonel/WaiterSalesDetail.sql"), System.Text.UTF8Encoding.Default);
                        }
                    }
                    else if (AppDbType == "3")
                    {
                        if (AppDbTypeStatus == "True")
                        {
                            ...
                            //        " LEFT JOIN F0" + FirmaId + "TBLSTOKLAR AS STK ON FSH.STOKIND=STK.IND WHERE FSH.ISLEMTARIHI>=@Trh1 AND FSH.ISLEMTARIHI<=@Trh2 AND FIRMAIND= @FirmaInd  AND ISNULL(FSB.IADE,0)=0 " +
                            ...
                            Query =
                                  ...
                                  " select  " +
                                  " SUM(T.SAY) ISLEMSAYISI, " +
                                  " SUM(T.MIKTAR) MIKTAR, " +
                                  " SUM(T.TUTAR) TUTAR, " +
                                  " T.PERSONEL " +
                                  " FROM( " +
                                  " (SELECT " +
                                  " (SELECT count(BASLIKIND) FROM TBLFASTERSATISBASLIK WHERE BASLIKIND = FSH.BASLIKIND) SAY, " +
                                  ...
                                  " GROUP BY FSH.PERKODU, FSH.BASLIKIND) " +
                                  "   ) AS T " +
                                  " GROUP BY " +
                                  " T.PERSONEL ";
                        ...
                        {
                            //Query = File.ReadAllText(HostingEnvironment.MapPath("/Sql/SqlFaster/PersonelSatis/PersonelSatisFasterOFLINE2.sql"), System.Text.UTF8Encoding.Default);24.03.2020 değiştrldi.
