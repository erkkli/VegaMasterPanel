...
{
    public class SendMailUrunGrubuDetailCRDU
    {
        public static List<UrunGrubuDetailSendMailViewModel> List(DateTime Date1, DateTime Date2, string subeid, string productGroup, string ID)
        {

            List<UrunGrubuDetailSendMailViewModel> Liste = new List<UrunGrubuDetailSendMailViewModel>();
                        ...
                        try
                        {
                            ...
                            if (UrunGrubuDt.Rows.Count > 0)
                            {
                                if (subeid.Equals(""))
                                {
                                    ...
                                    //List<UrunGrubu> CiroyaGoreListe = list.OrderByDescending(o => o.Ciro).ToList();
                                    UrunGrubuDetailSendMailViewModel items = new UrunGrubuDetailSendMailViewModel();
                                    items.Sube = SubeAdi;
                                    //items.SubeID = Convert.ToInt32(SubeId);
                                    //foreach (DataRow SubeR in UrunGrubuDt.Rows)
                                    //{
