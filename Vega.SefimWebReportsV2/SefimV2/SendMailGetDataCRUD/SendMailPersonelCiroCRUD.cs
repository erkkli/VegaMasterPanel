...
{
    public class SendMailPersonelCiroCRUD
    {
        public static List<PersonelCiroReportSendMailViewModel> List(DateTime Date1, DateTime Date2, string subeid, string ID)
        {
            List<PersonelCiroReportSendMailViewModel> Liste = new List<PersonelCiroReportSendMailViewModel>();
                ...
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    if (ID == "1")
                    {
                        ...
                        try
                        {
                            ...
                            if (subeid.Equals(""))
                            {
                                PersonelCiroReportSendMailViewModel items = new PersonelCiroReportSendMailViewModel();
                            ...
                            else
                            {
                                foreach (DataRow SubeR in AcikHesapDt.Rows)
                                {
                                    ...
                                    items.Sube = SubeAdi;
                                    //items.SubeID = Convert.ToInt32(SubeId);
                                    items.PersonelAdi = f.RTS(SubeR, "PersonelAdi");
                                    //items.Tutar = f.RTD(SubeR, "TUTAR");

                                    items.PersonelAdi = f.RTS(SubeR, "ReceivedByUserName");
                                    items.Nakit = f.RTD(SubeR, "CashPayment");
                                    items.Tutar = f.RTD(SubeR, "Total");
                                    items.Kredi = f.RTD(SubeR, "CreditPayment");
                                    items.YemekCeki = f.RTD(SubeR, "TicketPayment");
                        ...
                        foreach (var item in model.FR_SubeListesi)
                        {
                            if (item.SubeID == Convert.ToInt32(SubeId))
                            {
                                ...
                                catch (System.Exception ex)
                                {
                                    ...
                                    //items.SubeID = Convert.ToInt32(SubeId);
                                    items.PersonelAdi = "*";
                                    items.Tutar = 0;//f.RTD(SubeR, "TUTAR");
